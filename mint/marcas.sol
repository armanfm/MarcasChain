...
// Interface do MarcasNFT para consultar status
interface IMarcasNFT {
    function statusMarca(string memory nome) external view returns (string memory);
}
...
contract Marcas {
    ...
    address public contratoNFT; // MarcasNFT autorizado a liberar marcas
    IMarcasNFT public nft;      // referência para consulta de status
    ...
    // Define o contrato NFT autorizado a liberar marcas
    ...
    function analisar(string memory query) public view returns (
        ...
        string memory matchStatus  // status da marca com maior match: DISPONIVEL | RESERVADA | ATIVA
    ) {
        uint256 best = 0;
        ...
        // Consulta status do match no MarcasNFT (se configurado)
        string memory status = "";
        if (bytes(bestNome).length > 0) {
            if (address(nft) != address(0)) {
                try nft.statusMarca(bestNome) returns (string memory s) {
                    status = s;
                } catch {
                    status = "RESERVADA"; // tem no Marcas mas NFT nao consultavel
                }
            } else {
                status = "RESERVADA"; // registrada mas sem NFT configurado
            }
        }

        if (best >= 70) return (best, "REJECTED", "HIGH", bestNome, status);
        return (best, "APPROVED", "LOW", bestNome, status);
    }
    ...
    // ========== LIBERAR (chamada pelo MarcasNFT) ==========
    // Quando o NFT expira e é liberado, a marca volta a estar disponível
